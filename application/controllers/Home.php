...
parent::__construct();
	...
	$data['sum'] = $this->base_model->run_query("SELECT  a.ce , b.cj , c.ca , d.cr FROM (SELECT COUNT(employer.id) AS ce FROM employer) a , (SELECT COUNT(job.id) AS cj FROM job) b , (SELECT COUNT(applicant.id) AS ca FROM applicant) c , (SELECT COUNT(resume.id) AS cr FROM resume) d");
